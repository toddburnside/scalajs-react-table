...
object HTMLTableBuilder {
   ...
   *
   * @param tableMaker
   *   The TableMaker providing the useTable hook.
   * @param headerCellFn
   *   A function to use for creating the <th> elements. Simple ones are provided by the TableMaker
   *   object.
   * @param tableClass
   ...
   *
   * Notes: If optimization is required, parts of the options and the data should be memoized and
   * only change when necessary. See react-table documentation for details. The headerCellFn is
   * expected to return a <th> element. The function should make use of the column instance "props"
   * methods. Table footers are fully "Build It Yourself" since it is often useful for table footers
   * to span multiple columns and react-table does not support that. At some point, something
  ...
  /**
   * Create a virtualized table based on the configured plugins of a `TableMaker`. The plugins MUST
   * include withBlockLayout.
   *
   * @param tableMaker
   *   The TableMaker providing the useTable hook, configured with withBlockLayout.
   * @param options
   ...
   *   Relying on index does not work well for sortable tables since it is the unsorted index.
   *
   * Notes: Virtualized tables are not built from html table elements such as <table>, <tr>, etc.
   * The virtualization requires they be made from <div> elements and styled as a table. This method
   * adds a class to each div that corresponds to it's role, such as "table", "thead", "tr", etc.
   * The "tr" div is currently wrapped inside an additional div that handles the virtualization.
   * This means that headerCellFn must also return a <div> instead of a <td>, and should probably
   * also have a class of "td". The basic header functions in TableMaker take a boolean flag to
   * handle this.
   *
   * The body height must be set to a value either through the bodyHeight parameter or via CSS or
   * the body will collapse to nothing. In CSS, you MUST NOT use relative values like "100%" or it
   * won't work.
   */
  // format: off
